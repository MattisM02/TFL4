...
$configs = @(
  @{ id = "coops_on_default";  javaOpts = "" },
  @{ id = "coops_off";         javaOpts = "-XX:-UseCompressedOops" }
)
...
foreach ($cfg in $configs) {
  ...
  # Dokumentiere JVM-Mode vorab (separater Container, nur zum Nachweis)
  $tsCfg = Get-Date -Format "yyyy-MM-dd_HH-mm-ss"
  $cfgOut = "results\step_coops_${cfgId}_$tsCfg.txt"
  ...
  "--- verify compressed oops mode (XshowSettings:vm) ---" | Out-File -Append $cfgOut
  if ($javaOpt -ne "") {
    docker run --rm --memory $Mem -e JAVA_TOOL_OPTIONS="$javaOpt" --entrypoint java $Image -XshowSettings:vm -version 2>&1 | Out-File -Append $cfgOut
  } else {
    docker run --rm --memory $Mem --entrypoint java $Image -XshowSettings:vm -version 2>&1 | Out-File -Append $cfgOut
  }
  "" | Out-File -Append $cfgOut
  ...
  for ($n=1; $n -le $Runs; $n++) {
    ...
    $name = "demo-$cfgId-$n"
    $out  = "results\step_coops_${cfgId}_run${n}_$ts.txt"
...
"Done. Check .\results\step_coops_*"
